...
if (wishlistBtn) {
  ...
  // 🛡️ Защита от пустых значений
  if (!productId || !customerId) {
    console.warn("❌ Пропущен wishlist клик: нет productId или customerId", { productId, customerId });
    if (!customerId) showLoginModal(wishlistBtn);
    return;
  }
  ...
  try {
    ...
    buttons.forEach((btn) => {
      ...
      if (result.status === "added") {
        ...
        setTimeout(() => window.__wishlistRemovedCache.delete(productId), 5000);
      }
    });
